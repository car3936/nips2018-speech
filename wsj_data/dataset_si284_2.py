...
class WSJDataSet(object):
    ...
    def __init__(self, batch_size, charset, base_path,sample_rate=2, data_path = './data/', preprocessed = True, reduced = False):
        ...
        n_file_valid = len(self.valid_list) 
        ...
        for i in range(n_file_valid):
            l = self.valid_list[i]
            t = self.valid_label[i]
            if self.processed:
                n_frame = np.load(base_path + l[:-1]).shape[0]
                #n_frame_compressed = np.ceil(n_frame/4).astype('int32')
                n_frame_compressed = np.ceil(n_frame/sample_rate).astype('int32')
            ...
            if n_frame < 400 :
                valid_idx_small.append(i)

            if n_frame < 800 :
                valid_idx_mid.append(i)

            if n_frame < 1200 :
                valid_idx_big.append(i)

            if n_frame < 1600 :
                valid_idx_all.append(i)
        ...
        self.valid_idx_under_400 = np.asarray(valid_idx_small, dtype='int32')
        self.valid_idx_under_800 = np.asarray(valid_idx_mid, dtype='int32')
        self.valid_idx_under_1200 = np.asarray(valid_idx_big, dtype='int32')
        self.valid_idx_under_1600 = np.asarray(valid_idx_all, dtype='int32')
        ...
        if reduced:
            ...
            self.valid_idx_under_400 = self.valid_idx_under_400[::2]
            self.valid_idx_under_800 = self.valid_idx_under_800[::2]
            self.valid_idx_under_1200 = self.valid_idx_under_1200[::2]
            self.valid_idx_under_1600 = self.valid_idx_under_1600[::2]
        ...
        print('# of valid small dataset : ', self.valid_idx_under_400.shape[0])
        print('# of valid mid dataset : ', self.valid_idx_under_800.shape[0])
        print('# of valid big dataset : ', self.valid_idx_under_1200.shape[0])
        print('# of valid all dataset : ', self.valid_idx_under_1600.shape[0])
    ...
    def set_mode(self, mode):
        ...
        elif self.mode == 'valid_under_1600':
            self.n_data = self.valid_idx_under_1600.shape[0]
        elif self.mode == 'test':
    ...
    def get_data(self):
        ...
        # no shuffle for valid and test
            ...
            data_idx = self.valid_idx_under_1600[data_idx]
        elif self.mode == 'test':
