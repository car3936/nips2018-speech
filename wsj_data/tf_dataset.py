...
class WSJDataSet(object):
    ...
    def __init__(self, batch_size, charset, base_path, sample_rate = 1, use_boundary = False, data_path = './data/', boundary_path = '~/segmentation_data', preprocessed = True):
        ...
        for i in range(len(charset)):
            self.char_to_label[charset[i]] = i
            self.label_to_char[i] = charset[i]
        ...
        # remove new line 
        self.train_list = [n[:-1] for n in self.train_list]
        self.valid_list = [n[:-1] for n in self.valid_list]
        self.test_list = [n[:-1] for n in self.test_list]

        import functools

        #train_indice, train_value, train_shape = map(functools.partial(label_to_sparse_tensor, charset=self.char_to_label),self.train_label)
        train_shape = list(map(functools.partial(label_to_sparse_tensor, charset=self.char_to_label),self.train_label))
        train_indice, train_value, train_shape = map(list, zip(*train_shape))
        print(len(train_indice))
        print(len(train_indice[0]))
        #print(train_shape[0])

        filenames = tf.constant(self.train_list)

        self.train_dataset = tf.data.Dataset.from_tensor_slices(filenames)
        #self.train_label_dataset = tf.data.Dataset.from_tensor_slices((train_indice, train_value, train_shape))
        self.train_label_dataset = tf.data.Dataset.from_tensor_slices(np.asarray(train_indice))
    ...
    def get_data(self):
        ...
        for i in data_idx:
            use_boundary = False
            if self.mode == 'valid':
                file_name = self.valid_list[i]
                target_string = self.valid_label[i]
            elif self.mode == 'test':
                file_name = self.test_list[i]
                target_string = self.test_label[i]
            else: # training
                file_name = self.train_list[i]
                target_string = self.train_label[i]
                if self.boudnary_path is not None:
