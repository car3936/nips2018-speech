...
class Model_CTC_SRU(object):
    ...
    def __init__(self, stochastic = False, use_slope = True, variational_dropout = False, vocabulary_size=283, label_size=50,  rnnSize=256, n_layers=3, dropout=0.5, embedding_size=None, dtype=tf.float32, clip = 0.35, k_width=3, name = 'hlstm', conv_filter=5, batch_size = 128):
...
#        h = tf.reshape(h, [h_shape[0], h_shape[1], 320])
        h = tf.reshape(h, [batch_size, h_shape[1], 320])
        sru_ = SRU_layer(self.rnnSize, batch_size=self.batch_size, fwidth=self.k_width, pool_type = 'fo', zoneout=self.zoneout, name='QRNN_layer0', infer = tf.logical_not(self.is_train),skip = True, skip_embedding = True)
        sru_h, last_state = sru_(h)

#        cell = SRUCell(self.rnnSize,  initializer = tf.variance_scaling_initializer(1.0, 'fan_out', 'uniform'), first_layer = True,  use_skip = True)
        #cell = LSTMCell(self.rnnSize)
#        cell = DropoutWrapper(cell, output_keep_prob = dropout_p, variational_recurrent = variational_dropout, dtype=self.dtype)

#        self.lstm_cells.append(cell)

        for i in range(1,self.n_layers ):
            sru_ = SRU_layer(self.rnnSize, batch_size=self.batch_size, fwidth=self.k_width, pool_type = 'fo', zoneout=self.zoneout, name='QRNN_layer{}'.format(i), infer = tf.logical_not(self.is_train),skip = True, skip_embedding = False)
            sru_h, last_state = sru_(sru_h)
            #cell = LSTMCell(self.rnnSize,  initializer = tf.variance_scaling_initializer(1.0, 'fan_out', 'uniform'), forget_bias=f_bias)
...
#            _h, last_state = tf.nn.dynamic_rnn(
#                cell = self.lstm_cells[0],
#                inputs = h,
#                dtype = self.dtype
#            )
...
#        self.g_norm_sum = tf.reduce_mean(g_norm)
#        self.g_loss_sum = tf.reduce_mean(gate_loss)
        #output LSTM
#        self.output_lstm = SRUCell(self.rnnSize, initializer = tf.variance_scaling_initializer(1.0, 'fan_out', 'uniform'))
        #self.output_lstm = LSTMCell(self.rnnSize)
#        self.output_lstm = DropoutWrapper(self.output_lstm, output_keep_prob = dropout_p, variational_recurrent=variational_dropout, dtype=self.dtype)
#        output_lstm_input = lstm_input
#        print(output_lstm_input)

#        with tf.variable_scope('lstm_output'):
#            output_h, last_state = tf.nn.dynamic_rnn(
#                    cell = self.output_lstm,
#                    inputs = output_lstm_input,
#                    dtype=self.dtype
#                )
