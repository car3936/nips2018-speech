...
class Model_CTC_LSTM(object):

    def __init__(self, stochastic = False, use_slope = True, variational_dropout = False, vocabulary_size=283, label_size=50,  rnnSize=256, n_layers=3, dropout=0.5, embedding_size=None, dtype=tf.float32, clip = 0.35,  name = 'hlstm'):
        ...
        h = tf.layers.conv2d(self.x, 32, (5,5), (2,2), 'same', use_bias= False, name = 'conv0')
        ...
        h = tf.layers.conv2d(h, 32, (5, 5), (1, 2), 'same', use_bias=False, name = 'conv1')
        ...
        cell = LSTMCell(self.rnnSize,  initializer = tf.variance_scaling_initializer(1.0, 'fan_out', 'uniform') )
...
#                self.h.append(_h)
        ...
        #output LSTM
        self.output_lstm = LSTMCell(self.rnnSize, initializer = tf.variance_scaling_initializer(1.0, 'fan_out', 'uniform'))
        ...
        # logit of softmax 
        print(output_h)
        h_shape = tf.shape(output_h)
        output_h = tf.reshape(output_h, [-1, self.rnnSize])
        print(output_h)
