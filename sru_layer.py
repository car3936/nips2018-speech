...
class QRNN_pooling(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, out_fmaps, pool_type, skip):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or "QRNN-{}-pooling".format(pool_type)):
            if pool_type == 'f':
                ...
                # jhpark - added X for highway connection
                if self._skip:
                    Z, F, O, X = tf.split(inputs, 4, 1)
                else:
                    Z, F, O = tf.split(inputs, 3, 1)
                new_state = tf.multiply(F, state) + tf.multiply(tf.subtract(1., F), Z)
                if self._skip:
                    output = tf.multiply(O, new_state) + tf.multiply(tf.subtract(1., O), X ) 
                else: 
        ...
        with tf.variable_scope(self.name):
            ...
            if self.skip:
                 if self.skip_embedding:
                     input_embedding = tf.layers.dense(input_, self.out_fmaps, use_bias=False)
                     T = tf.concat([T] + [input_embedding], 2)
                 else:
                     T = tf.concat([T] + [input_], 2 )
                     pass

            # create the pooling layer
            pooling = QRNN_pooling(out_fmaps, pool_type, self.skip)
    ...
    def convolution(self, input_, filter_width, out_fmaps, pool_type, zoneout_):
        ...
        with tf.variable_scope('convolutions'):
            ...
            for gate_name in pool_type:
                ...
                if zoneout_ > 0 and gate_name == 'f': # now self.infer is tensor 
                    print('Applying zoneout {} to gate F'.format(zoneout_))
                    # appy zoneout to F
                    g = tf.cond(self.infer, lambda: g, lambda: zoneout((1. - g), 1. - zoneout_))
